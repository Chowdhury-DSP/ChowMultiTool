...
mkdir -p bin/Win64
mkdir -p bin/Win32
declare -a plugins=("ChowMultiTool")
...
(
    cd bin
    rm -f "ChowMultiTool-Win64-${VERSION}.zip"
    zip -r "ChowMultiTool-Win64-${VERSION}.zip" Win64
    # rm -f "ChowMultiTool-Win32-${VERSION}.zip"
    # zip -r "ChowMultiTool-Win32-${VERSION}.zip" Win32
)
