...
if [ "$1" == "build" ]; then
xcodebuild -project build-ios/ChowMultiTool.xcodeproj \
  -scheme ChowMultiTool_Standalone archive -configuration Release \
  -sdk iphoneos -jobs 12 -archivePath ChowMultiTool.xcarchive | xcpretty
fi
fi
...
if [ "$1" == "version" ]; then
  ...
  PLIST=ChowMultiTool.xcarchive/Info.plist
  ...
  mkdir -p "$archive_dir"
  mv ChowMultiTool.xcarchive "$archive_dir/ChowMultiTool-$COMMIT.xcarchive"
fi
